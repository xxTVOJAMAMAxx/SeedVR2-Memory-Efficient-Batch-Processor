...
class BatchFrameToDiskSaver:
    ...
    def save_batch_to_disk(self, processed_batch, batch_index, total_batches, start_frame_number, 
        ...
        # Debug info
        print(f"🔍 Debug: Batch {batch_index}, start_frame_number={start_frame_number}, "
              f"skip_frames={skip_frames}, save_start_frame={save_start_frame}, "
              f"frames_to_save={len(frames_np)}")
        
        # Save frames with multi-threading
        if image_format == "WebP":
            saved_count = self.save_frames_webp_threaded(frames_np, output_path, save_start_frame)
        else:  # PNG
            saved_count = self.save_frames_png_threaded(frames_np, output_path, save_start_frame)
        
        self.last_saved_frame = save_start_frame + saved_count - 1
    ...
    def save_frames_webp_threaded(self, frames_np, output_path, start_frame_number):
    ...
    def save_frames_png_threaded(self, frames_np, output_path, start_frame_number):
